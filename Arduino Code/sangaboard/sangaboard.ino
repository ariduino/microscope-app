 ...
 *
 * LED serial commands (RGBW NeoPixel):
 * - led <r> <g> <b> <w> <brightness> : set LED colour channels and global brightness (0..255 each), apply to all pixels, and save to EEPROM.
 * - led?                              : print current LED state as "led r g b w brightness".
 * - led_off                           : set LED to off (0 0 0 0) at brightness 0 and save to EEPROM.
 * - led_default                       : set LED to default white state (0 0 0 255 255) and save to EEPROM.
 */
...
// NeoPixel RGBW illumination settings
#if defined(SANGABOARDv3)
// A5 is used for motor control on v0.3/v0.4 layouts, so pick a safe default.
...
LedState led_state = { 0, 0, 0, 255, 255 };
const uint8_t led_eeprom_signature = 0x5A;
const int led_state_eeprom = sizeof(long) * (n_motors + 5);
const int led_signature_eeprom = led_state_eeprom + sizeof(LedState);
...
void apply_led_state() {
  strip.setBrightness(led_state.brightness);
  uint32_t color = strip.Color(led_state.r, led_state.g, led_state.b, led_state.w);
...
void print_led_state() {
  Serial.print("led ");
  Serial.print(led_state.r);
  Serial.print(" ");
  Serial.print(led_state.g);
  Serial.print(" ");
  Serial.print(led_state.b);
  Serial.print(" ");
  Serial.print(led_state.w);
  Serial.print(" ");
  Serial.println(led_state.brightness);
}

void save_led_state() {
  EEPROM.put(led_state_eeprom, led_state);
  EEPROM.put(led_signature_eeprom, led_eeprom_signature);
}
...
// get the stepoper objects from the motor shield objects
  ...
  strip.begin();
  uint8_t saved_signature = 0;
  EEPROM.get(led_signature_eeprom, saved_signature);
  if (saved_signature == led_eeprom_signature) {
    EEPROM.get(led_state_eeprom, led_state);
  } else {
    save_led_state();
  }
  apply_led_state();
...
void loop() {
  ...
  if (received_bytes > 0) {
    ...
    if (command.startsWith("led_default")) {
      led_state.r = 0;
      led_state.g = 0;
      led_state.b = 0;
      led_state.w = 255;
    ...
    if (command.startsWith("led ")) {
      int parsed[5];
      int parsed_count = sscanf(input_buffer, "led %d %d %d %d %d", &parsed[0], &parsed[1], &parsed[2], &parsed[3], &parsed[4]);
      if (parsed_count != 5) {
        Serial.println("Error: command is led <r> <g> <b> <w> <brightness>");
        return;
      }
      for (int i = 0; i < 5; i++) {
        if (parsed[i] < 0 || parsed[i] > 255) {
          Serial.println("Error: LED values must be in range 0..255");
          return;
        }
      }
      led_state.r = (uint8_t)parsed[0];
      led_state.g = (uint8_t)parsed[1];
      led_state.b = (uint8_t)parsed[2];
      led_state.w = (uint8_t)parsed[3];
      led_state.brightness = (uint8_t)parsed[4];
      apply_led_state();
      save_led_state();
      Serial.println("done.");
      return;
    ...
    if (command.startsWith("help")) {
      ...
      Serial.println(F("led <r> <g> <b> <w> <bri>      - set LED colour and global brightness (0..255 each)"));
      Serial.println(F("led?                           - print LED state as: led r g b w brightness"));
      Serial.println(F("led_off                        - set LED state to 0 0 0 0 0 and save"));
      Serial.println(F("led_default                    - set LED state to 0 0 0 255 255 and save"));
#ifdef LIGHT_SENSOR
